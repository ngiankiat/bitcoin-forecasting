...
bitcoin_model <- bitcoin_price %>%
  ...
  mutate(future_return=lead(future_return,2),
...
test <- bitcoin_model %>%
  filter(between(date, as.Date("2020-01-01"), as.Date("2019-11-29"))) %>%
  mutate(bad = ifelse(is.na(bad), 0, bad),
         good = ifelse(is.na(good), 0, good),
         overall = good - bad)
...
train_predict <- train %>%
  ...
  mutate(signal = ifelse(.pred_1 > 0.5, 1, 0),
         trading_cost = abs(signal - lag(signal,1, default=0)) * 0.003,
         return_buyhold = cumprod(1 + future_return),
         return_buy_model = cumprod(1 + future_return * signal - trading_cost),
         return_buy_sell_model = cumprod(1 + ifelse(signal==1,
                                                    future_return - trading_cost,
                                                    -future_return - trading_cost)))
  
# just to visualise part of the train_predict tibble
train_predict %>%
  select(date, open, close, future_return,
         signal, trading_cost,
         return_buyhold, return_buy_model,
         return_buy_sell_model)
#%>%
  #head(10)
```
...
ggplot(train_predict, aes(x = date)) + 
  ...
  geom_line(aes(y = return_buy_model), colour = "blue") +
  geom_line(aes(y= return_buy_sell_model), colour="green") +
  #scale_y_log10() +
  ylab("Returns")
```
