...
bitcoin_model <- bitcoin_price %>%
  ...
  select(date, close, volume_btc, volume_currency, future_return, future_return_sign)
...
# leading the future_return_sign so that we do not run on today's price
train <- bitcoin_model %>%
  filter(between(date, as.Date("2018-01-01"), as.Date("2020-11-29"))) %>%
  mutate(future_return_sign = lead(future_return_sign),
         bad = ifelse(is.na(bad), 0, bad),
         good = ifelse(is.na(good), 0, good),
         overall = good - bad) %>%
  .[-nrow(.), ] # remove last row, because y variable was brought forward by 1 day
...
The ensemble has performed relatively well, giving an accuracy of 82.6%. However, 1 key issue might be a realtively high number of false postive results and it is possible that days with huge falls in bitcoin pricing could affect the returns/