...
**Currently these are the additional features added: (updated as of 28/11)**
...
5. EMA
6. Rate of Change (it was mentioned that this can be combined with other indicators e.g. moving averages or stochastic to provide a better analysis?)
7. On-Balance Volume
8. Network Value to Transactions (NVT) - something similar to "P/E ratio"
...
## **EMA**
More weightage placed on recent data compared to using SMA

```{r}
bitcoin_date <- as.data.frame(bitcoin_price$date)
colnames(bitcoin_date) <- c("date")
ema <-EMA(Cl(bitcoin_price),n=20) %>% as.data.frame()
ema <- bitcoin_date %>% cbind(ema)
colnames(ema) <- c("date","EMA")
