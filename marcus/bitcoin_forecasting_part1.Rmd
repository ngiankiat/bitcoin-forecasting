...
**Currently these are the additional features added: (updated as of 01/12)**
...
7. CNY/USD 
8. Some of the more important bitcoin (hashrate) - will see what else in the bitcoin_data df is useful
...
### **(TO IGNORE): Bitcoin News Articles - Sentiments** 
...
```{r}
plot1 <- ggplot(bitcoin_model, aes(x = date)) + geom_line(aes(y = close, colour = "Bitcoin Closing Price")) + 
  scale_colour_manual(values = c("darkred"))
plot2 <- ggplot(bitcoin_model, aes(x = date)) +  geom_line(aes(y = tweets_volume, colour = "Twitter Volume")) +
...
```{r}
# BCHAIN/HRATE
bitcoin_hr <- select(bitcoin_data, c(date,bchain.hrate))
bitcoin_model <- bitcoin_model %>% left_join(bitcoin_hr)
```
