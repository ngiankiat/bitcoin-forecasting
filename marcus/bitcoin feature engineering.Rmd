...
A paper uses the following 3 below: [(Poyser, 2017)](https://arxiv.org/ftp/arxiv/papers/1706/1706.01437.pdf)
...
* When %B is above 1, price is above the upper band and when %B is below 0, price is below the lower band. [(S,2020)](https://school.stockcharts.com/doku.php?id=technical_indicators:bollinger_band_perce)
...
* Usually Short Period (fast) = 12days, Long Period (slow) = 26days, nSig(k) = 9days. [(Yu, 2020)](https://bookdown.org/kochiuyu/technical-analysis-with-r-second-edition/macd.html)
* Since it was mentioned that traders **use the MACD’s histogram** (which is MACD - signal) to identify when bullish or bearish momentum is high, so we will only extract the distance calculated (MACD-signal) column to add into the bitcoin model. [(Fernando, 2020)](https://www.investopedia.com/terms/m/macd.asp)
...
[(Mitchell, 2020)](https://www.investopedia.com/terms/a/accumulationdistribution.asp)
...
[(Resta, Pagnottoni & De Giuli, 2020)](https://www.mdpi.com/2227-9091/8/2/44)
...
[(Mitchell, 2020)](https://www.investopedia.com/terms/p/pricerateofchange.asp)
...
[(Menchero & Nagy, 2020)](https://www.ambroker.com/en/analysis/blog/what-rate-change-roc-indicator-and-how-use-trading/)
...
[(O,2020)](https://blog.okcoin.com/2020/07/17/top-bitcoin-price-indicators-metrics-traders-need-to-know/)
...
[(Interdax, 2020)](https://help.interdax.com/hc/en-001/articles/360002605137-What-is-the-Network-Value-to-Transactions-NVT-Ratio-)
...
[(Hayes, 2020)](https://www.investopedia.com/terms/a/atr.asp)
...
* Besides all the data that's scrapped, a research paper [(Zhu, Yechen, Dickinson, David & Jianjun, 2017)](https://www.econstor.eu/bitstream/10419/176446/1/10.1186_s40854-017-0054-0.pdf) has revealed that Federal Funds Rate is a possible influence on Bitcoin prices. 
...
* It is considered because it might have an impact on Bitcoin. E.g. 12 March 2020, the price of Bitcoin fell below USD 4,000 after a sharp decline in the S&P500 Index in the U.S.
...
# Include s&p500 returns into the model as well
bitcoin_model <- bitcoin_model %>% tq_mutate(select = sp500,
            mutate_fun = periodReturn,
            period = 'daily',
            type = 'arithmetic',                            
...
[Site](https://www.financialresearch.gov/financial-stress-index/)
...
### **CNY/USD**
* Bitcoin is mostly traded in China [(Oyedele, 2017)](https://www.businessinsider.com/bitcoin-trading-china-yuan-remnibi-2017-1)
* Chinese mining pools control more than 60% of the Bitcoin network’s collective hashrate [(Gogo, 2020)](https://news.bitcoin.com/65-of-global-bitcoin-hashrate-concentrated-in-china/) and one-third of global hash rate is in Xinjiang.  [(Haig, 2020)](https://cointelegraph.com/news/a-single-chinese-region-accounts-for-one-third-of-global-btc-hash-rate)
* So it might be worthwhile to account for movements in the exchanges between the dollar with the yuan 
* Bitcoin is used as a vehicle for capital flight out of China. Savers and investors frequently use the cryptocurrency as an alternative holding asset against a weakening Yuan. There is also evidence of a significant negative correlation between Yuan and Bitcoin prices. [(Marshall, 2019)](https://cointelegraph.com/explained/how-china-influences-bitcoin-price-explained)
* Historical trends appear to suggest that the crypto markets do pay attention to global events and geopolitical tensions.
* US-China trade war impact the CNY/USD currencies, which in turn may benefit Bitcoin. [(Freitas, 2020)](https://currency.com/china-relationship-to-bitcoin-value-coronavirus-and-us-trade-deal) & [(Huang, 2020)](https://www.forbes.com/sites/rogerhuang/2020/05/10/bitcoin-might-benefit-from-a-us-china-cold-war/?sh=2c3e457315db)
...
multiple_crypto_indexes <- crypto_index_df %>% left_join(index_bitx)

ggplot(multiple_crypto_indexes, aes(x = date)) + 
  geom_line(aes(y = crypto_index, colour = "Our Crypto Index")) + ylab("Own Index") +
  geom_line(aes(y = bitx/200,colour = "KEYS:SW")) +
  scale_y_continuous(sec.axis = sec_axis(~.*200, name = "Bitwise Index"))
...
13. Interdax. (2020, July 01). What is the Network Value to Transactions (NVT) Ratio? Retrieved from https://help.interdax.com/hc/en-001/articles/360002605137-What-is-the-Network-Value-to-Transactions-NVT-Ratio-
