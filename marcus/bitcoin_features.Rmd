...
* A study showed that the number of tweets on Twitter can influence BTC trading volume for the following day which can affect price fluctuations and number of transaction of cryptocurrencies. [Reference: Shen D, Urquhart A, Wang P (2019) Does twitter predict Bitcoin? Econ Lett 174:118–122]

```{r, warning=FALSE}
# load the csv first
twitter_volume <- read.csv("twitter_volume.csv")
...
```{r, warning=FALSE}
bitcoin_google <- read.csv("bitcoin_google.csv")
...
```{r, warning=FALSE}
# load the csv first
xau_usd_historical <- read.csv("xau_usd_historical.csv")
...
us_dollar_index_historical_data <- read.csv("us_dollar_index_historical_data.csv")
...
[Reference](https://blog.shrimpy.io/blog/cryptocurrency-index-funds-step-by-step-guide)
...
```{r, warning=FALSE}
top10_crypto <- read.csv("top10_crypto.csv")
...
# bchain.atrct - Bitcoin Median Transaction Confirmation Time
# load in the data first
median_confirmation_time_bitcoin <- read.csv("median-confirmation-time-bitcoin.csv")
