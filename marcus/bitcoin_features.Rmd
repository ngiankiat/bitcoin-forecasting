...
We have created a simple crypto index based on weighted market cap of top 10 crypto. 
...
* Secondly, Allocation Distribution: 
    ...
    + Continuous improvement of this index will be necessary. Currently, it will just serve as a benchmark for comparison.
...
#lets assume the base of this index is 10000, so the
divisor = subset(mkt_cap_total, date == "2018-01-01")$sqrt_sum_mkt_cap/10000
...
crypto_index_df <- select(crypto_index, c(date,crypto_index_return))
...
```{r}
ggplot(mkt_cap_total, aes(x = date)) + 
  geom_line(aes(y = index), colour = "blue", alpha = 0.8)
```
