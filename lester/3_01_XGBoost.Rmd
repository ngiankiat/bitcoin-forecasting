...
Next, we create resamples of the training data for cross-validation. We set a 3-month window for resmaples, such that the model can be tuned every 3 months. This simulates our model being updated every 3 months in response to changing conditions.
...
We visualise the importance of 
...
Although we observe an expected decrease in prediction accuracy from training to testing data, the 43% accuracy is still quite reasonable.
...
# write_csv(rf_final_testing_pred %>% select(-future_return_sign), "../data/xgboost_signals.csv")
```

# 14. Resampling (on Entire Set)

We perform another round of resampling, this time using 1-month intervals. 

## 14.1 Define resamples from train + test
```{r}
...
computeReturns <- function(base, predicted){
  overall <- base %>%
    left_join(predicted, by="date") %>%
    mutate(return_model = pred_class * future_return)
