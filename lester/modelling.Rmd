...
test <- bitcoin_model %>% filter(between(date, as.Date("2020-01-01"), as.Date("2020-11-29")))
```
...
# 7. Create grid for tuning. Train and Tune Model.
```{r}
...
# 9. Finalize Model with the best model parameters
```{r}
...
final_tree_training_pred <- 
  predict(final_tree, train) %>% 
  # Add in predicted class probabilities
  bind_cols(predict(final_tree, train, type = "prob")) %>% 
...
final_tree_testing_pred <- 
  predict(final_tree, test) %>% 
  # Add in predicted class probabilities
  bind_cols(predict(final_tree, test, type = "prob")) %>% 
  # Add the true outcome data back in
