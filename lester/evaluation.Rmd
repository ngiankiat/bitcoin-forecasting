...
prophet_plot <- all_models %>% ggplot(aes(x = date, y = close, color = signal_prophet)) +
    geom_line() +
    theme_light()
xbg_plot <- all_models %>% ggplot(aes(x = date, y = close, color = signal_xgb)) +
    geom_line() +
    theme_light()
ensemble_plot <- all_models %>% ggplot(aes(x = date, y = close, color = signal_ensemble)) +
    geom_line() +
    theme_light()
grid.arrange(prophet_plot, xbg_plot, ensemble_plot, ncol = 1)
```
