...
prophet_forecasts_sign <-
    prophet_forecasts %>%
    mutate(pred_signal = ifelse(prophet_forecasts$price > lag(prophet_forecasts$price), 
                                       1, 0)
    ) %>%
...
test_and_prophet <- 
  ...
  mutate(trading_cost = abs(pred_signal - lag(pred_signal, n = 1, default = 0)) * 0.003,
         ...
         return_prophet = cumprod(1 + future_return * pred_signal - trading_cost))
...
prophet_forecasts_sign %>%
  ...
  write.csv(file="../data/prophet_signals.csv")
```
